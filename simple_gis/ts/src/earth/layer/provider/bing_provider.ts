 ...
 */
function numerationSystemTo10(numSys: number, strNum: string): number{
  let sum = 0;
 ...
 */
function numerationSystemFrom10(numSys: number, num: number): string{
  var tempResultArray = [];
...
* @returns {string}
*/
function numerationSystemChange(numSysFrom: number, numSysTo: number, strNumFrom: string): string{
...
export default class BingProvider extends Provider{
	public getUrl(column: number, row: number, level: number): string{
    var url = "";
    var tileX = column;
    var tileY = row;
